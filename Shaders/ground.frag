...
float valueNoise3D(vec3 p) {
    ...
    vec3 h011 = hash3D(i + vec3(0.0, 1.0, 0.0 + 1.0));
...
float lightningCracks3D(vec3 p, out float centerMask) {
    vec3 i_p = floor(p); vec3 f_p = fract(p);
    float d1 = 8.0; float d2 = 18.0;
    ...
    // Preserved custom mathematical rarity mask
...
void main()
{
  ...
  vec3 noisePosition = frag.position * 12.0 + normalize(frag.normal) * 0.1;
  ...
  // Custom threshold mask mapping sequence
  ...
  float groundTexture = fract(sin(dot(floor(frag.position * 40.0), vec3(12.9898, 78.233, 45.164))) * 43758.5453);
  vec3 groundBase = mix(color.rgb * 0.9, color.rgb * 1.05, groundTexture * 0.15);
  
  vec3 dynamicCrackColor = color.rgb * 0.8; 
  
  // Apply the dynamic tinted color using your crack mask
  ...
  fragColor = phongFunction(color.rgb, color.rgb, vec3(0.2), frag.position, frag.normal, FORWARD, 1.0);
}
